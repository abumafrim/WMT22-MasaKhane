...
def train(config,n_epochs,optimizer,tokenizer,train_dataset,dev_dataset,model,save_with_bt=False):
  ...
  for epoch_idx in range(n_epochs):
    if epoch_idx>=config.state_dict_check['epoch']+1:
        ...
        for batch_idx, (input_batch, label_batch) in tqdm(enumerate(data_generator), total=n_batches):
            if input_batch is None and label_batch is None:
              continue
...
def main(args):
    ...
    if os.path.exists(os.path.join(config.model_path_dir,config.model_name)):
        ...
        try:
            ...
            with open(config.log,'a+') as fl:
                print(f"No {config.model_name.split('.')[0]+'_bt.pt'} present. Default to original {config.model_name}",file=fl)
    ...
    if config.do_backtranslation:
        ...
        for n_bt in range(config.NUM_BACKTRANSLATION_TIMES):
            if n_bt>=config.state_dict_check['bt_time']+1:
                ...
                train(config,config.n_bt_epochs,optimizer,tokenizer,augmented_dataset,dev_dataset,model,save_with_bt=True)
